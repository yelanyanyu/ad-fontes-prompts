...
### 3. 数据管理器 (The Manager)
如果你想把学过的词都存起来，建立自己的语料库，用这个。它包含一个极简的 Web 界面和 PostgreSQL 数据库后端。
*   **文件**：`start_tool_yml2pg.bat`
*   **用法**：**双击脚本**。
    *   它会自动启动服务并打开浏览器 (http://localhost:3000)。
    *   你可以在网页上录入、保存、查看和删除单词。
    *   *注：初次使用需要确保本地安装了 PostgreSQL，脚本会自动尝试建表。*
